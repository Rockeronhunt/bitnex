...
// which are part of the BITNEX repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="bitnex-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /bitnex-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "bitnexpay-bitnex/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from bitnexpay-bitnex/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'bitnexpay-bitnex/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from bitnexpay-bitnex/develop"
            }
          }

          if (hasCache) {
            sh "cd /bitnex-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /bitnex-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /bitnex-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /bitnex-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /bitnex-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /bitnex-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /bitnex-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
