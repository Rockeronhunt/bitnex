...
# bitnex_hash
RUN git clone https://github.com/bitnexpay/bitnex_hash
RUN cd bitnex_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} bitnex
RUN useradd -u ${USER_ID} -g bitnex -s /bin/bash -m -d /bitnex bitnex
...
RUN mkdir /bitnex-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /bitnex-src && \
