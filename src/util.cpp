...
const char * const BITCOIN_CONF_FILENAME = "bitnex.conf";
const char * const BITCOIN_PID_FILENAME = "bitnexd.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL || *cacheCounter != logAcceptCategoryCacheCounter.load())
        {
            ...
            if (mapMultiArgs.count("-debug")) {
                ...
                // "bitnex" is a composite category enabling all BITNEX-related debug output
                if(ptrCategory->count(std::string("bitnex"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BITNEX
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BITNEX
    // Mac: ~/Library/Application Support/BITNEX
    // Unix: ~/.bitnex
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BITNEX";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/BITNEX";
#else
    // Unix
    return pathRet / ".bitnex";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty bitnex.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override bitnex.conf
