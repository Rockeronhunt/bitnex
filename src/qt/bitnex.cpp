...
{
    return QCoreApplication::translate("bitnex-core", psz).toStdString();
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in bitnex.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in bitnex.qrc)
...
/** Class encapsulating BITNEX Core startup and shutdown.
...
/** Main BITNEX application object */
...
void BitcoinApplication::initializeResult(bool success)
{
    ...
    if(success)
    {
        ...
        // bitnex: URIs or payment requests:
...
{
    QMessageBox::critical(0, "Runaway exception", BitcoinGUI::tr("A fatal error occurred. BITNEX Core can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(EXIT_FAILURE);
}
...
int main(int argc, char *argv[])
{
    ...
    Q_INIT_RESOURCE(bitnex);
    Q_INIT_RESOURCE(bitnex_locale);
    ...
    /// 6. Determine availability of data directory and parse bitnex.conf
    ...
    // bitnex: links repeatedly have their payment requests routed to this process:
