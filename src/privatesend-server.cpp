...
void CPrivateSendServer::ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if (fLiteMode) return; // ignore all BITNEX related functionality
...
/*
    ...
    allow endless transaction that would bloat BITNEX and make it unusable. To
...
{
    if (fLiteMode) return;        // disable all BITNEX specific functionality
